...
def create_dataloader(
    ...
    return (
        loader(
            ...
            sampler=sampler,
            pin_memory=True,
            collate_fn=LoadImagesAndLabelsAndClasses.collate_fn4
            if quad
            else LoadImagesAndLabelsAndClasses.collate_fn,
        ),
        dataset,
    )
...
class LoadImagesAndLabelsAndClasses(Dataset):
    ...
    def __init__(
        ...
        labels, shapes, self.segments, self.gt_classif = zip(*cache.values())
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        assert (
            classes_df.shape[1] == 2
        )  # 2 columns, 1st one is the image path and the second one is the ground truth class
        count_na = 0
        for key in x.keys():
...
def dataset_stats(
    ...
    for split in "train", "val", "test":
        if data.get(split) is None:
            ...
            continue
        x = []
        dataset = LoadImagesAndLabelsAndClasses(data[split])  # load dataset
