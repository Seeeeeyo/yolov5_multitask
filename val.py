...
def run(
    ...
    for batch_i, (im, targets_det, paths, shapes, targets_cls) in enumerate(pbar):
        ...
        if compute_loss:
            loss = compute_loss(
                ([x.float() for x in pred_det_layers_out], pred_cls),
                targets_det.to(device),
                targets_cls.to(device),
            )
            loss_det += loss[2]
            loss_cls += loss[3]
        ...
        lb = (
            [targets_det[targets_det[:, 0] == i, 1:] for i in range(nb)]
            if save_hybrid
            else []
        )  # for autolabelling
        t3 = time_sync()
        out_det = non_max_suppression(
            pred_det,
            conf_thres,
            iou_thres,
            labels=lb,
