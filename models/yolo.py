...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], multitasks=False)  # model, savelist
...
class HybridModel(BaseModel):
    # YOLOv5 detection model
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, nc_cls=None):  # model, input channels, number of classes
        super().__init__()
        if isinstance(cfg, dict):
            self.yaml = cfg  # model dict
        else:  # is *.yaml
        ...
        if anchors:
            LOGGER.info(f'Overriding model.yaml anchors with anchors={anchors}')
            self.yaml['anchors'] = round(anchors)  # override yaml value
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], multitasks=True)  # model, savelist
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names
...
def parse_model(d, ch, multitasks=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(to_enumerate):  # from, number, module, args
...
if __name__ == '__main__':
    ...
    model = HybridModel(opt.cfg).to(device)
